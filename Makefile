.PHONY: deploy infra ping provision destroy help

.DEFAULT_GOAL := help

deploy: infra ping provision ## Run full deployment workflow: Terraform -> Wait for SSH -> Ansible

infra: ## Provision LXC container in Proxmox and generate inventory
	@echo "==> [1/3] Provisioning container with Terraform..."
	cd terraform-proxmox && terraform init && terraform apply -auto-approve

ping: ## Poll until the container SSH service is reachable
	@echo "==> [2/3] Waiting for SSH service to become ready..."
	@cd ansible-proxmox && \
...
provision: ## Install and configure Docker CE and plugins via Ansible
	@echo "==> [3/3] Installing Docker CE via Ansible..."
